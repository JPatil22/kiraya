-- ============================================================================
-- _0035_apply.sql — paste this into the Supabase SQL editor.
--
-- Idempotent: rebuilds v_listings_public to expose sourced_broker_name — the
-- source_name from listing_sources — so a listing seeded from an outside post
-- can credit the real broker on the public page. Safe to run more than once:
-- `drop view if exists` + recreate no-ops on a second run.
--
-- Exposes ONLY the name. source_phone and the private note stay out of every
-- public read; the number reaches a tenant solely through contact unlock. This
-- is a deliberate departure from 0034's private-by-default stance — see
-- 0035_public_source_name.sql for the reasoning. Definition is 0033's, verbatim,
-- plus one lateral join and one column.
-- ============================================================================
