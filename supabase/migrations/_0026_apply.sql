-- ============================================================================
-- Kiraya - migration 0026.
--
--   0026  SOMEWHERE TO SEND IT: profiles.email (optional; phone stays the
--         identity bar) and notifications.emailed_at (the delivery queue),
--         plus the trigger that makes 0012's promise true - marking read
--         really is the only update anyone may make, now that emailed_at is a
--         column worth lying about.
--
-- Your database already has 0001-0025. Paste into the Supabase SQL editor and
-- Run. Safe to run more than once.
--
-- Nothing is sent by the database. Delivery is a route in the app -
-- /api/notifications/deliver - because the provider key belongs in the app's
-- environment, not in Postgres. See .env.example for the three variables it
-- needs; without them the route returns 503 rather than pretending.
-- ============================================================================
