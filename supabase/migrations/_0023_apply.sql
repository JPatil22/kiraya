-- ============================================================================
-- Kiraya — migration 0023.
--
--   0023  BROKERAGE DISCLOSURE: `brokerage` has been `not null default 0`
--         since 0002, so a broker who never touched the field produced a
--         listing that looked fee-free and then asked for a month's rent at
--         the door. A zero fee is now a claim someone has to make, an owner
--         listing cannot carry one at all, and both rules are enforced by a
--         trigger that — unlike properties_guard — also binds service-role
--         writes, which is what open mode makes.
--
-- Your database already has 0001–0022. Paste into the Supabase SQL editor and
-- Run. It drops and recreates v_listings_public, which is expected, and is
...
alter table public.properties
  add column if not exists brokerage_disclosed boolean not null default false;
...
drop trigger if exists properties_brokerage_guard_biu on public.properties;
