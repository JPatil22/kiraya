-- ============================================================================
-- _0033_apply.sql — paste this into the Supabase SQL editor.
--
-- Idempotent: adds property_photos.thumbnail_path if missing and rebuilds
-- v_listings_public to expose the cover thumbnail. Safe to run more than once —
-- `add column if not exists` and `drop view if exists` + recreate both no-op on
-- a second run. See 0033_photo_thumbnails.sql for why the feed needs a small
-- image the detail page does not.
-- ============================================================================
